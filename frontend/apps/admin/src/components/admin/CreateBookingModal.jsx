    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div className="relative flex max-h-[94vh] w-full max-w-6xl min-w-0 flex-col overflow-hidden rounded-3xl border border-sky-100 bg-white shadow-2xl" initial={{ scale: 0.96, y: 18 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 18 }}>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4 pb-28 sm:p-6 sm:pb-32">
              {body()}
            </div>
          </motion.div>
        </motion.div>
