...
export default function AdminAnalyticsPage() {
  ...
  const analyticsFilters = useMemo(() => analyticsRangeQuery(range), [range]);
  const { data: dashboard, isLoading: loadingDashboard } = useAdminStats(analyticsFilters);
  const { data: overview, isLoading: loadingOverview } = useAdminAnalyticsOverview(analyticsFilters);
  ...
  return <div className="flex flex-col gap-6 text-left">
    ...
    <AdminStatGrid>
      ...
      <Metric icon={TrendingUp} label="Range Growth" value={`${growth.growthPercentage || 0}%`} />
    </AdminStatGrid>
    ...
    <Card className="p-6 bg-bg-card border border-bg-border/60 glass flex flex-col gap-4">
      <div className="flex flex-col text-left">
        ...
        <span className="text-[10px] text-text-tertiary">Filtered by selected analytics range</span>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card className="p-5 bg-bg-card border border-bg-border/60 glass text-left">
        ...
        <div className="grid gap-3 text-sm">
          <Row label="Selected range" value={formatCurrency(growth.currentPeriodRevenue || 0)} />
          <Row label="Previous matching range" value={formatCurrency(growth.previousPeriodRevenue || 0)} />
