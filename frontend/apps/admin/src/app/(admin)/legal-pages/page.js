...
export default function LegalPagesAdmin() {
  ...
  return <div className="space-y-6">
    ...
    <section className="admin-summary-row grid grid-cols-3 gap-2 sm:gap-4"><Stat label="Configured" value={`${pages.length}/4`} icon={FileText} /><Stat label="Published" value={pages.filter((page) => page.isPublished !== false).length} icon={Eye} green /><Stat label="Still to create" value={Math.max(0, 4 - usedTypes.size)} icon={FilePlus2} /></section>
