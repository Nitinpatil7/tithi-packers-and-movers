...
export default function LegalPagesAdmin() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-3"><Stat label="Configured" value={`${pages.length}/4`} icon={FileText} /><Stat label="Published" value={pages.filter((page) => page.isPublished !== false).length} icon={Eye} green /><Stat label="Still to create" value={Math.max(0, 4 - usedTypes.size)} icon={FilePlus2} /></section>
