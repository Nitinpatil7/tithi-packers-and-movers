...
function AddonEditor({ record, uploadIcon, onClose, onSave, busy }) {
  ...
  return (
    <Modal isOpen onClose={onClose} title={record._id ? 'Edit add-on' : 'Create add-on'} size="lg">
      <form onSubmit={(event) => { event.preventDefault(); const payload = { ...form, icon: String(form.icon || '').trim() || null, price: Number(form.price) || 0 }; delete payload.sortOrder; onSave(payload); }} className="space-y-4">
              ...
              <div className="min-w-0">
                ...
                <div className="max-h-72 space-y-3 overflow-y-auto pr-1">
                  {isLoadingItems ? (
                    ...
                    return (
                      <div key={group.id}>
                        <h4 className="sticky top-0 z-10 mb-2 flex items-center justify-between gap-2 rounded-xl border border-sky-100 bg-white px-3 py-2 text-xs shadow-sm">
                          <span className="min-w-0"><span className="block truncate font-black text-text-primary">{group.name}</span><span className="mt-0.5 block truncate font-semibold text-primary">{group.section}</span></span>
