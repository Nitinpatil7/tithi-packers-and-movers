...
export default function AdminTestimonialsPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="admin-summary-row grid grid-cols-3 gap-2 sm:gap-4"><Stat label="Total records" value={testimonials.length} icon={MessageSquareQuote} /><Stat label="Active" value={testimonials.filter((item) => item.status === 'active').length} icon={Eye} green /><Stat label="Featured" value={testimonials.filter((item) => item.isFeatured).length} icon={Star} /></section>
