...
export default function AdminItemsPage() {
  ...
  const { data: pricingRules = [] } = useAdminPricingRules({ isActive: true });
  ...
  return <div className="items-manager space-y-7 text-left">
    ...
    {isLoading ? <Empty loading /> : isError ? <Empty text="Could not load the item catalog." action={() => refetch()} /> : !sections.length ? <Empty text="No sections yet. Create the first section to start your catalog." /> : <section className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm">
...
function RateAdjustmentManager({ open, pricingRules, mutation, onClose }) {
  const serviceRules = useMemo(() => pricingRules.filter((rule) => rule.isActive !== false), [pricingRules]);
  const firstRuleId = serviceRules[0]?._id || '';
  ...
  useEffect(() => {
    ...
    if (open && selectedRuleId && !serviceRules.some((rule) => rule._id === selectedRuleId)) setSelectedRuleId(firstRuleId);
  }, [firstRuleId, open, selectedRuleId, serviceRules]);
  useEffect(() => {
    if (!open) return;
    setForm(editing ? {
      name: editing.name || '',
  ...
  if (!open) return null;
  const selectedRule = serviceRules.find((rule) => rule._id === selectedRuleId) || serviceRules[0];
...
function Stat({ icon: Icon, label, value }) { return <div className="admin-summary-card rounded-2xl border border-sky-100 bg-white p-3 sm:p-5"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-semibold uppercase leading-tight tracking-wide text-slate-400 sm:text-xs">{label}</span><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-sky-50 sm:h-9 sm:w-9"><Icon className="h-4 w-4 text-sky-600 sm:h-4.5 sm:w-4.5" /></span></div><strong className="mt-2 block text-2xl font-bold text-sky-700 sm:mt-3 sm:text-3xl">{value}</strong></div>; }
function Empty({ text, action, loading = false }) { return <div className="grid min-h-44 place-items-center rounded-2xl border border-dashed border-sky-200 p-10 text-center text-sm font-semibold text-slate-400">{loading ? <Spinner size="md" /> : <span>{text}{action && <button onClick={action} className="ml-2 font-bold text-sky-600">Try again</button>}</span>}</div>; }
