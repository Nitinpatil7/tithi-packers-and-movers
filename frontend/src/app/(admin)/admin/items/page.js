...
import { DEFAULT_ITEM_ICON, ITEM_ICON_OPTIONS, ItemIcon } from '@/lib/itemIcons';
  ...
  const saveEditor = async (form) => {
    try {
      if (editor.type === 'section') {
        ...
        const payload = { groupId: form.groupId || editor.group?._id, name: form.name.trim(), icon: form.icon || DEFAULT_ITEM_ICON, sizes, sortOrder: Number(form.sortOrder) || 0, isActive: form.isActive };
                    ...
                    <div>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-start gap-2">
                          ...
                          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-sky-50 text-sky-600 ring-1 ring-sky-100"><ItemIcon icon={item.icon} className="h-4.5 w-4.5" /></span>
...
function RecordEditor({ editor, sections, globalSizes, busy, onClose, onSave }) {
  ...
  useEffect(() => {
    if (!editor) return;
    if (editor.type === 'item') setForm({ name: record?.name || '', icon: record?.icon || DEFAULT_ITEM_ICON, groupId: record?.groupId || editor.group?._id || '', sortOrder: record?.sortOrder || 0, isActive: record?.isActive !== false, sizes: (record?.sizes || []).map((size) => ({ sizeId: sizeIdOf(size), price: size.price, isActive: size.isActive !== false })) });
