...
import { CircleHelp, Edit3, Eye, EyeOff, Plus, Search, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden border-sky-100 bg-white">
        ...
        {isLoading ? <div className="p-12 text-center text-sm text-slate-500">Loading FAQs...</div> : isError ? <div className="p-12 text-center"><p className="text-sm text-red-500">Could not load FAQs.</p><button onClick={() => refetch()} className="mt-3 text-sm font-bold text-sky-600">Try again</button></div> : filtered.length === 0 ? <div className="p-12 text-center text-sm text-slate-500">No FAQs found.</div> : (
          <div className="divide-y divide-sky-50">{filtered.map((faq) => <div key={faq._id} className="flex flex-col gap-4 p-5 hover:bg-sky-50/40 lg:flex-row lg:items-start">
            <div className="flex min-w-0 flex-1 gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-sky-100 text-sky-600"><CircleHelp className="h-5 w-5" /></span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-slate-900">{faq.question}</h3><span className="rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-bold uppercase text-sky-700">{faq.category || 'general'}</span>{faq.isActive === false ? <span className="flex items-center gap-1 text-xs font-bold text-slate-400"><EyeOff className="h-3.5 w-3.5" /> Inactive</span> : <span className="flex items-center gap-1 text-xs font-bold text-emerald-600"><Eye className="h-3.5 w-3.5" /> Active</span>}</div><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-500">{faq.answer}</p><p className="mt-2 text-xs font-semibold text-slate-400">Sort order: {faq.sortOrder || 0}</p></div></div>
