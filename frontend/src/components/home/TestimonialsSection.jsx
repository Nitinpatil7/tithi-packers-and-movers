...
function TestimonialCard({ item, active }) {
  return <article className={`group flex h-[342px] w-[86vw] max-w-[390px] shrink-0 snap-center flex-col justify-between rounded-3xl border bg-white p-6 transition-all duration-300 sm:w-[360px] md:p-7 ${active ? 'border-orange-200 shadow-xl sm:-translate-y-1' : 'border-sky-100 shadow-card hover:border-orange-100 hover:-translate-y-1 hover:shadow-md active:scale-[.99]'}`}>
...
export default function TestimonialsSection() {
  ...
  return <section className="dotted-light-bg relative overflow-hidden py-20 md:py-32"><div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-bg-border to-transparent" /><Image src="/back_truck.png" alt="" width={600} height={400} className="pointer-events-none absolute -left-32 top-24 z-0 w-[320px] opacity-35 drop-shadow-[0_22px_34px_rgba(15,23,42,.16)] sm:-left-40 sm:top-24 sm:w-[550px] md:top-16 md:w-[480px] md:opacity-55 lg:-left-44 lg:w-[580px] lg:opacity-80" /><div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <motion.header initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto mb-12 flex max-w-2xl flex-col items-center gap-4 text-center"><span className="section-label bg-white/90"><MessageSquareQuote className="h-3.5 w-3.5 text-orange-500" /> Verified customer reviews</span><h2 className="mt-2 text-display-md font-black text-[#063642] md:text-display-lg">Words of appreciation from <span className="text-orange-500">our customers</span></h2><p className="text-base font-medium leading-7 text-text-secondary md:text-lg">Real experiences from families and businesses who trusted Tithi with their move.</p>{!displayLoading && <div className="mt-2 inline-flex items-center gap-4 rounded-2xl border border-orange-100 bg-white/90 px-5 py-3 shadow-card"><strong className="text-2xl text-[#063642]">{average}</strong><div className="flex gap-0.5">{[1,2,3,4,5].map((star) => <Star key={star} className="h-3.5 w-3.5 fill-orange-500 text-orange-500" />)}</div><span className="border-l border-orange-100 pl-4 text-xs font-bold text-text-secondary">{testimonials.length} published reviews</span></div>}</motion.header>
