...
export default function ItemSelectionStep({ onSubmit, onBack, initialData = {}, isIntercity = false }) {
  ...
  return <div className="booking-items-ui space-y-7 pb-24 text-left sm:pb-4">
    <header><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-primary-soft text-primary"><Package className="h-5 w-5" /></span><div><h3 className="text-2xl font-black text-text-primary">Select Items to Move</h3><p className="mt-1 text-sm font-medium text-text-secondary">Choose a section, open a group, and add the items you want to move.</p></div></div></header>
    ...
    {isLoading ? <div className="grid min-h-72 place-items-center rounded-3xl border border-bg-border"><Spinner size="md" /></div> : isError ? <div className="rounded-3xl border border-red-200 bg-red-50 p-10 text-center"><p className="text-sm font-bold text-red-600">Could not load moving items.</p><button onClick={() => refetch()} className="mt-3 text-sm font-black text-primary">Try again</button></div> : !sections.length ? <div className="rounded-3xl border border-dashed border-bg-border p-10 text-center text-sm font-semibold text-text-secondary">No moving items are currently available.</div> : <div className="grid gap-7 lg:grid-cols-12 lg:items-start">
      <main ref={sectionTopRef} className="space-y-5 scroll-mt-28 lg:col-span-8"><div className="flex gap-2 overflow-x-auto pb-1">{sections.map((entry) => <button key={entry._id} onClick={() => goToSection(entry._id)} className={`shrink-0 rounded-xl px-4 py-2.5 text-sm font-black transition ${section?._id === entry._id ? 'bg-primary text-white shadow-sky' : 'border border-bg-border bg-bg-white text-text-secondary hover:border-primary/30 hover:text-primary'}`}>{entry.name}</button>)}</div>
        ...
        <div className="space-y-3">{groups.map((group) => { const open = expandedGroups[group._id] || Boolean(search); return <section key={group._id} className="overflow-hidden rounded-2xl border border-bg-border bg-bg-white"><button type="button" onClick={() => setExpandedGroups((value) => ({ ...value, [group._id]: !open }))} className="flex w-full items-center justify-between gap-3 p-4 text-left hover:bg-bg-section/50"><span className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-primary-soft text-primary"><Boxes className="h-4 w-4" /></span><span><strong className="block text-sm text-text-primary">{group.name}</strong><small className="font-semibold text-text-tertiary">{group.items?.length || 0} choices</small></span></span>{open ? <ChevronDown className="h-4 w-4 text-primary" /> : <ChevronRight className="h-4 w-4 text-text-tertiary" />}</button>{open && <div className="grid gap-2 border-t border-bg-border bg-bg-section/20 p-3 sm:grid-cols-2">{group.items?.map((item) => { const size = primarySize(item); if (!size) return null; const key = itemKey(item._id, size); const qty = quantity(key); return <article key={item._id} className={`booking-item-row flex items-center justify-between gap-3 rounded-2xl border bg-bg-white p-3 transition ${qty ? 'border-primary/30 bg-primary-soft/60' : 'border-bg-border hover:border-primary/20'}`}><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary-soft text-primary"><ItemIcon icon={item.icon} className="h-5 w-5" /></span><h4 className="truncate text-sm font-semibold text-text-primary">{item.name}</h4></div>{qty ? <div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => changeQuantity(item, size, group, -1)} className="grid h-8 w-8 place-items-center rounded-lg border border-bg-border bg-bg-white text-text-primary"><Minus className="h-3.5 w-3.5" /></button><strong className="w-5 text-center text-sm font-semibold text-primary">{qty}</strong><button type="button" onClick={() => changeQuantity(item, size, group, 1)} className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-white"><Plus className="h-3.5 w-3.5" /></button></div> : <button type="button" onClick={() => changeQuantity(item, size, group, 1)} className="shrink-0 rounded-lg border border-primary/20 px-3 py-1.5 text-xs font-semibold text-primary transition hover:bg-primary hover:text-white">Add</button>}</article>; })}</div>}</section>; })}{!groups.length && <div className="rounded-2xl border border-dashed border-bg-border p-10 text-center text-sm font-semibold text-text-tertiary">No matching groups or items.</div>}</div>
    ...
    <footer className="fixed inset-x-0 bottom-0 z-30 border-t border-bg-border bg-bg-white/95 px-3 py-2 shadow-[0_-16px_40px_rgba(15,23,42,.10)] backdrop-blur sm:sticky sm:bottom-4 sm:rounded-2xl sm:border sm:px-4 sm:py-3">
      <div className="mx-auto grid max-w-5xl grid-cols-3 items-center gap-2 sm:flex sm:justify-between sm:gap-3">
        ...
        <button type="button" onClick={handlePrimaryAction} className="btn-sky flex min-h-12 items-center justify-center gap-1.5 rounded-2xl px-3 text-xs font-semibold sm:hidden">{nextSection ? `Next: ${nextSection.name}` : 'Continue'}<ArrowRight className="h-4 w-4" /></button>
      </div>
    </footer>
  </div>;
}
