...
import { ArrowRight, ChevronDown, ChevronRight, Minus, Package, Plus, Search, ShieldCheck, ShoppingCart, Trash2 } from 'lucide-react';
import Spinner from '@ui/Spinner';
import { useItemCatalog, useItemSections, usePrefetchItemCatalog } from '@hooks/useItems';
...
export default function ItemSelectionStep({ onSubmit, onBack, initialData = {}, isIntercity = false }) {
  const { data: sectionList = [], isLoading: sectionsLoading, isError: sectionsError, refetch: refetchSections } = useItemSections();
  ...
  const prefetchItemCatalog = usePrefetchItemCatalog();

  const sections = useMemo(() => Array.isArray(sectionList) ? sectionList : [], [sectionList]);
  ...
  const { data: catalogSections = [], isLoading: catalogLoading, isFetching: catalogFetching, isError: catalogError, refetch: refetchCatalog } = useItemCatalog(
    { sectionId: activeSection },
    { enabled: Boolean(activeSection) }
  );
  const catalogSection = catalogSections.find((entry) => entry._id === activeSection) || catalogSections[0] || null;
  const section = catalogSection || sections.find((entry) => entry._id === activeSection) || sections[0];
  const isLoading = sectionsLoading || (Boolean(activeSection) && catalogLoading && !catalogSection);
  const isError = sectionsError || catalogError;
  const refetch = () => {
    if (sectionsError) refetchSections();
    if (catalogError) refetchCatalog();
  };
  ...
  useEffect(() => {
    if (!nextSection?._id) return;
    prefetchItemCatalog({ sectionId: nextSection._id });
  }, [nextSection?._id, prefetchItemCatalog]);
  ...
  return <div className="booking-items-ui space-y-7 pb-24 text-left sm:pb-4">
    ...
    {isLoading ? <div className="grid min-h-72 place-items-center rounded-3xl border border-bg-border"><Spinner size="md" /></div> : isError ? <div className="rounded-3xl border border-red-200 bg-red-50 p-10 text-center"><p className="text-sm font-bold text-red-600">Could not load moving items.</p><button onClick={() => refetch()} className="mt-3 text-sm font-black text-primary">Try again</button></div> : !sections.length ? <div className="rounded-3xl border border-dashed border-bg-border p-10 text-center text-sm font-semibold text-text-secondary">No moving items are currently available.</div> : <div className="grid gap-7 lg:grid-cols-12 lg:items-start">
      <main ref={sectionTopRef} className="min-w-0 space-y-5 scroll-mt-32 lg:col-span-8"><div className="booking-category-tabs scrollbar-none flex w-full max-w-full snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain pb-2 pr-[18vw] sm:pr-2">{sections.map((entry) => <button key={entry._id} onClick={() => goToSection(entry._id)} className={`min-h-11 w-auto shrink-0 snap-start whitespace-nowrap rounded-2xl px-4 py-2.5 text-sm font-black transition active:scale-[.98] ${section?._id === entry._id ? 'bg-primary text-white shadow-sky' : 'border border-bg-border bg-bg-white text-text-secondary hover:border-primary/30 hover:text-primary hover:shadow-xs'}`}>{entry.name}</button>)}</div>
