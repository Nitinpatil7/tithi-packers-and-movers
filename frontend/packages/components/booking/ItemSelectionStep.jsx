  ...
  const changeQuantity = (catalogItem, size, group, increment) => {
    ...
    setSelectedItems((current) => {
      ...
      if (index < 0 && increment > 0) return [...current, {
        ...
        section: section?.name, sectionId: section?._id, groupId: group._id, group: group.name,
...
function ItemCartBar({ totalCount, cartOpen, onViewCart, onCloseCart, onProceed, actionLabel, cartGroups }) {
  return (
    <>
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-slate-900/45 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Selected item cart" onClick={onClose}>
      ...
      <div className="absolute inset-x-0 bottom-0 flex max-h-[50svh] flex-col overflow-hidden rounded-t-3xl bg-bg-white shadow-[0_-24px_70px_rgba(15,23,42,.22)]" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
          {groups.map((section) => (
            <section key={section.key} className="space-y-2">
              <h4 className="text-sm font-semibold text-text-secondary">{section.name}</h4>
              <div className="rounded-2xl border border-sky-100 bg-bg-section/80 p-2 shadow-xs dark:bg-bg-muted">
                {section.groups.map((group) => (
                  <div key={group.key} className="space-y-1.5 border-b border-bg-border/60 py-2 first:pt-0 last:border-b-0 last:pb-0">
                    <h5 className="px-2 text-[11px] font-semibold uppercase tracking-wide text-primary">{group.name}</h5>
                    {group.items.map((item) => (
                      <div key={item.itemKey || item.key || item.name} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-xl border border-sky-100 bg-white px-2.5 py-2 shadow-xs dark:border-slate-700 dark:bg-slate-800">
                        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-primary-soft/35 text-primary">{item.icon ? <CatalogIcon icon={item.icon} alt="" className="h-8 w-8" sizes="32px" /> : <Package className="h-4 w-4" />}</span>
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-medium text-text-primary">{cleanText(item.name)}</span>
                        </span>
