...
function CatalogIcon({ icon, alt = '', className = 'h-5 w-5' }) {
  return icon ? (
    <Image
      src={icon}
      alt={alt}
...
export default function ItemSelectionStep({ onSubmit, onBack, initialData = {}, isIntercity = false }) {
  ...
  const iconFor = (entry) => entry?.icon || '';
